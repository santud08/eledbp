...
export const editMovieTmdbImages = async (
  data,
  titleId,
  imageType,
  createdBy,
  ...
  try {
    if (data && data != null && data.length > 0) {
      ...
      for (const tmdbImgData of data) {
        const getImgDataFile = await model.titleImage.findOne({
          attributes: ["id", "path"],
          where: {
            title_id: titleId,
        ...
        if (!getImgDataFile) {
          //Insert title background image into title image table for english language
          const createEnData = {
