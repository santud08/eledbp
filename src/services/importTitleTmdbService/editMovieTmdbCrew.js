...
export const editMovieTmdbCrew = async (data, titleId, createdBy, siteLanguage = "en") => {
  try {
    if (data && data != null && data.length > 0) {
      let payload = null;
      let primaryDetailsPayload = null;
      let payloadPeopleList = [];
      for (const castData of data) {
        ...
        const tmdbId = castData.tmdb_id ? castData.tmdb_id : null;
        let actionDate = "";

        const getPeople = await model.people.findOne({
          attributes: ["id"],
          where: { tmdb_id: tmdbId, status: { [Op.ne]: "deleted" } },
        });
        let peopleId = 0;
        ...
        if (getPeople) {
          ...
          const getPeopleData = await tmdbService.fetchPeopleDetails(tmdbId, siteLanguage);
      ...
      if (payloadPeopleList.length > 0) {
        ...
        schedulerJobService.addJobInScheduler(
          ...
          "search_db",
          `add people in edit movie tmdb crew`,
          createdBy,
        );
      }
