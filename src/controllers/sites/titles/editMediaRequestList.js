...
/**
 * editMediaRequestList
 * @param req
 * @param res
 */
export const editMediaRequestList = async (req, res, next) => {
  try {
    ...
    if (titleType === "tv" && Object.keys(condition).length > 0) {
      findMediaDetails = await model.titleRequestMedia.findOne({
        where: condition,
        status: "active",
      });
    ...
    if (foundMedia != null && foundMedia > 0) {
      ...
      if (findRequestSeasonId && findRequestSeasonId.season_id && titleType == "tv") {
        ...
        const tvSeasonIdCheck = titleType === "tv" ? findRequestSeasonId.season_id : "";
        const imageCategory =
          mediaType === "poster"
        ...
        titleTmdb = await model.title.findOne({
          attributes: ["tmdb_id"],
      ...
      if (seasonPkId && titleType == "tv") {
        ...
        const tvSeasonIdCheck = titleType === "tv" ? seasonPkId : "";
        const imageCategory =
          mediaType === "poster"
            ? { image_category: "poster_image" }
            : mediaType === "image"
