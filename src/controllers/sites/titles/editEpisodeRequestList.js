...
/**
 * editEpisodeRequestList
 * @param req
 * @param res
 */
export const editEpisodeRequestList = async (req, res, next) => {
  try {
    ...
    tmdbId = titleData && titleData.tmdb_id ? titleData.tmdb_id : "";
    let condition = {};
    if (draftSeasonId) {
      condition = {
          ...
          for (const value of episodeResultData) {
            const element = {
              ...
              description:
                ...
                value.episodeTranslations[0] &&
                value.episodeTranslations[0].dataValues &&
                value.episodeTranslations[0].dataValues.description
                  ? value.episodeTranslations[0].dataValues.description
                  : "",
              poster:
            ...
            for (const value of filterValue) {
              if (value) {
                let requiredFormat = {
                  ...
                  episode_summary: value.description ? value.description : "",
    ...
    res.ok({ draft_relation_id: relationId, results: data, tmdb_id: tmdbId });
  } catch (error) {
    next(error);
  }
};
