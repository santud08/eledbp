...
/**
 * seasonWebtoonsMediaDetails
 * @param req
 * @param res
 */

export const seasonWebtoonsMediaDetails = async (req, res, next) => {
  try {
    ...
    const getMedia = await model.title.findOne({
      ...
      where: { id: mediaId, type: "webtoons", record_status: "active" },
      include: [
        {
    ...
    const isEdit = await userPermissionService.checkEditorPermission(req, mediaId, "webtoons");
    ...
    res.ok({
      ...
      season_name:
        ...
        getMedia.dataValues.seasons[0].dataValues.seasonTranslations[0].season_name
          ? getMedia.dataValues.seasons[0].dataValues.seasonTranslations[0].season_name
          : "",
      webtoons_season_meadia_list: resultData.rows,
    });
  } catch (error) {
    next(error);
  }
