 ...
 * @param res
 */
export const saveEpisodeDetails = async (req, res, next) => {
  try {
    let data = {};
    ...
    // tmdbId for other language data insert from tmdb API
    ...
    if (!findEpisodeId) {
      ...
      if (otherLangSeasonReqId && tmdbId) {
        ...
        if (tmdbEpisodeData && tmdbEpisodeData.results && tmdbEpisodeData.results.length > 0) {
          for (const value of tmdbEpisodeData.results) {
            const element = {
              id: "",
              name: value.episode_name ? value.episode_name.trim() : "",
              description: value.overview ? value.overview : "",
              poster: value.poster_image ? value.poster_image : "",
              release_date: value.release_date ? value.release_date : "",
              title_id: "",
        ...
        for (const appendData of resultArrayObj) {
          if (appendData.action_type === "a" && parsedObject != null) {
            const appendElement = {
              ...
              description: appendData.description ? appendData.description : "",
