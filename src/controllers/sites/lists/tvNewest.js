 ...
 * @param res
 */
export const tvNewest = async (req, res, next) => {
  try {
    ...
    let timSpanDate = null;
    if (reqDate) {
      [getCurrentDate, timSpanDate] = await Promise.all([
    ...
    const attributes = ["title_id", "release_date", "release_date_to"];
    includeQuery = [
      {
        model: model.title,
        left: true,
    ...
    searchParams.sortOrderObj = [["release_date", "DESC"]];
    ...
    if (resultData.count > 0) {
      ...
      for (const eachRow of resultData.rows) {
        if (eachRow) {
          ...
          const getTvReleaseDate = await model.season.findOne({
            where: {
              title_id: title_id,
              status: "active",
              release_date: {
          ...
          const record = {
            ...
            release_date:
              getTvReleaseDate && getTvReleaseDate.release_date
                ? await customDateTimeHelper.changeDateFormat(
                    getTvReleaseDate.release_date,
                    "MMM DD,YYYY",
                  )
                : "",
          };
