...
export const esWebtoonsDocument = async (req, res, next) => {
  try {
    let webtoonsDataset = [];
    let titleCount = 0;
    const offset = req.body.offset ? req.body.offset : 0;
    const limit = req.body.limit ? req.body.limit : 10;
    const indexName = !req.body.index_name ? "search-webtoons" : req.body.index_name;

    const webtoonsData = await model.title.findAll({
    ...
    if (getData.length > 0) {
      for (const value of getData) {
        ...
        if (value.weeklyTelecasts.length > 0) {
          for (const weeklyTelecast of value.weeklyTelecasts) {
        ...
        webtoonsResultData.release_date = reReleaseDate
          ...
          ? value.release_date
          : null;
        webtoonsResultData.title_status = value.title_status ? value.title_status : null;

        webtoonsResultData.weekly_telecast =
          weeklyTelecastData && weeklyTelecastData.length > 0
            ? weeklyTelecastData.toString()
            : null;
        // -----------------------------sorting fields
        ...
        webtoonsDataset.push(webtoonsElement);
      }
    }

    if (webtoonsDataset.length > 0) {
      ...
      const titleDataSet = webtoonsDataset;
