...
// Define a function to process a YouTube URL.
export const updateYoutubeVideo = async () => {
  try {
    const todayDate = await customDateTimeHelper.getCurrentDateTime("YYYY-MM-DD");
    const getData = await model.schedulerJobs.findOne({
      where: {
        status: "pending",
        type: "update_youtube_video",
        [Op.and]: Sequelize.where(Sequelize.fn("date", Sequelize.col("created_at")), todayDate),
      },
    });
    if (getData && getData != null && getData != "undefined") {
      ...
      //service call
      const jres = await schedulerJobService.updateVideoDetailsFromYouTube(
      ...
      if (getData && getData != null && getData != "undefined") {
        if (
          (await customDateTimeHelper.changeDateFormat(getData.created_at, "YYYY-MM-DD")) <
          ...
          console.log(
            `${consoleColors.fg.red}  schedule ${getData.type}-${getData.id} is already created in schedule-data \n ${consoleColors.reset}`,
          );
        }
      } else {
        const limit = YOUTUBE_APIS.PER_BATCH;
        const offset = 0;
        const conditions = {
